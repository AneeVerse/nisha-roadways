...
const TestimonialCard: React.FC<{ t: Testimonial }> = ({ t }) => {
  return (
    <article className="relative h-full rounded-2xl sm:rounded-3xl lg:rounded-[28px] bg-white p-4 sm:p-6 lg:p-8 shadow-sm ring-1 ring-black/5 card-clip">
      <div className="mb-3 sm:mb-4 flex items-center gap-1">
        {Array.from({ length: 5 }).map((_, idx) => (
          <Star key={idx} filled={idx < t.rating} />
        ))}
      </div>
      <p className="text-slate-600 leading-6 sm:leading-7 text-sm sm:text-base">
        {t.quote}
      </p>
      <div className="mt-6 sm:mt-8 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm font-semibold text-slate-900">{t.name}</p>
...
const Testimonials: React.FC = () => {
  ...
  return (
    <section className="relative mx-auto max-w-[1400px] px-3 py-8 sm:px-4 sm:py-12 md:py-16 lg:px-8 bg-[#f4f6ff]">
      <div className="grid grid-cols-1 gap-6 sm:gap-8 lg:grid-cols-[300px_1fr] xl:grid-cols-[360px_1fr]">
        {/* Left dark panel */}
        <div className="rounded-2xl sm:rounded-3xl lg:rounded-[28px] bg-[#0a0f2a] text-white p-6 sm:p-8 flex flex-col justify-between min-h-[320px] sm:min-h-[400px] lg:min-h-[480px]">
          <div>
            <div className="mb-3 sm:mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-white/70">
              <Image src="/67761af397573ca47fb7f6cb_subtitle-icon.png" alt="Testimonials icon" width={16} height={16} />
              <span>Clients Testimonials!</span>
            </div>
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight font-[family-name:var(--font-space-grotesk)]">
              Clients Say
              <br />
          ...
          <div className="mt-6 sm:mt-8 lg:mt-10 flex items-center gap-3">
            <button
              ...
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg sm:rounded-xl bg-white text-slate-900 ring-1 ring-black/5 flex items-center justify-center hover:bg-slate-100 transition"
            >
            ...
            <button
              ...
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg sm:rounded-xl bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 transition"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="sm:w-[18px] sm:h-[18px]">
